...
">
                <div class="md:w-1/3 w-full">
                    <img class="rounded-lg shadow-lg antialiased" src="{{$company->profile()->getLogo()}}" alt="LOGO"
                         style="width: 50px">
                </div>
                <div class="md:w-2/3 w-full px-3 flex flex-row flex-wrap">
                    <div class="w-full text-right text-gray-700 font-semibold relative pt-3 md:pt-0">
                        <div class="text-2xl text-white leading-tight">{{$company->getName()}}</div>
                        <div class="text-normal text-gray-300"><span
                                class="border-b border-dashed border-gray-500 pb-1">price: {{$company->getPrice()}}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap">
                <form method="post" action="{{route('stock.buy', $company->getSymbol())}}">
                    ...
                    <button
                        ...
                        type="submit" value="buy">Buy stock
                    </button>
                </form>
                @error('credits')
